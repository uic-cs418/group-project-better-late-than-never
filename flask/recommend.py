...
def get_best_restaurants(df, category, neighborhood, price_range):
    ...
    tempDF["PriceRange"]=tempDF.PriceRange.astype('int')
    tempDF=tempDF[(tempDF["PriceRange"] <= price_range)]
    # select all restaurants in the appropriate neighborhood.
    
    # tempDF=tempDF[(tempDF["neighborhood"].str.contains(neighborhood, case=False, na=False))]
    
    # sort the resulting DF by average review score
    newDF=tempDF.sort_values(by=["average_stars"], inplace=False, ascending=False)
